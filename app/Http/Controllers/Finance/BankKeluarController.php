...
use App\Models\Finance\BankKeluar1;
use App\Models\Finance\BankKeluar2;
...
class BankKeluarController extends Controller
{
    public function show1(Request $request) {
        ...
        //var_dump(date($tgl1));
        $data['t_bank_keluar1']= BankKeluar1::from('t_bank_keluar1 as a')
    ...
    public function getAPInvoice(Request $request) {
        ...
        ->selectRaw('a.doc_key, string_agg(b.nm_bahan, \', \') AS catatan_dtl2')
        ->groupBy('a.doc_key');
        $subQ2= DB::table('t_ap_invoice3 as a')
        ->selectRaw('a.doc_key, string_agg(a.catatan, \', \') AS catatan_dtl3')
        ->groupBy('a.doc_key');
        $data['t_bank_keluar2']= BankKeluar2::from('t_ap_invoice1 as a')
        ->leftJoin('m_supplier as b','a.kd_partner','=','b.kd_supplier')
        ->leftJoin('m_supplier_grup as c','b.kd_supplier_grup','=','c.kd_supplier_grup')
        ->leftJoin('i_doctype as d','d.base_type','=',DB::raw(21)) //21=AP Invoice
        ->leftJoinSub($subQ1, 'e', function($join) {
            $join->on('a.doc_key', '=', 'e.doc_key');
        })
        ->leftJoinSub($subQ2, 'f', function($join) {
            $join->on('a.doc_key', '=', 'f.doc_key');
        })
        ->selectRaw("a.*, c.no_account AS no_account2, d.base_name as base_type_name,
            trim(
                COALESCE(e.catatan_dtl2,'') ||
                CASE WHEN e.catatan_dtl2 IS NOT NULL AND f.catatan_dtl3 IS NOT NULL THEN ', ' ELSE '' END ||
                COALESCE(f.catatan_dtl3,'')
            ) AS catatan_dtl")
    ...
    public function getAPCredit(Request $request) {
        $kd_partner=isset($request->kd_partner) ? $request->kd_partner : 0;
        $data['t_bank_keluar2']= BankKeluar2::from('t_ap_credit1 as a')
        ->leftJoin('m_supplier as b','a.kd_partner','=','b.kd_supplier')
        ->leftJoin('m_supplier_grup as c','b.kd_supplier_grup','=','c.kd_supplier_grup')
        ->leftJoin('i_doctype as d', function ($join) {
            $join->where('d.base_type', 22); //22=AP Credit
        })
        ->selectRaw("a.*, c.no_account AS no_account2, d.base_name as base_type_name")
        ->where("a.kd_partner",$kd_partner)
        ->whereRaw("COALESCE(a.rp_sisa,0) > 0")
    ...
    public function getBatal(Request $request) {
        ...
        ->where("a.doc_key",$doc_key)
        ->first();
        $response['value']= ($data['t_bank_keluar1']) ? $data['t_bank_keluar1']->fl_batal : 'false';
    ...
    public function getAllRef(Request $request) {
        ...
        //BankKeluar2
        $data['t_bank_keluar2']= BankKeluar2::from('t_bank_keluar2 as a')
    ...
    public function destroy(Request $request) {
        $doc_key=isset($request->doc_key) ? $request->doc_key : 0;
        BankKeluar3::where('doc_key',$doc_key)->delete();
        BankKeluar2::where('doc_key',$doc_key)->delete();
        BankKeluar1::where('doc_key',$doc_key)->delete();
    ...
    public function setBatal(Request $request) {
        ...
        $bankKeluar1= BankKeluar1::where('doc_key',$doc_key)->first();
        if ($bankKeluar1) {
            if ($bankKeluar1->fl_batal == 'true') {
            ...
            $bankKeluar1->batal_tgl = date('Y-m-d H:i:s');
            $bankKeluar1->batal_userid = $request->userid;
            $bankKeluar1->batal_lokasi = $request->lokasi;
            $bankKeluar1->save();
        }
    ...
    public static function updateLinkData($doc_key = 0, $insert = FALSE) {
        if ($insert == FALSE) {
            ...
            ->leftJoin("t_ap_invoice1 as b","a.base_ref","=","b.doc_key")
            ->selectRaw("b.doc_key, b.rp_sisa, a.rp_bayar, a.rp_diskon")
            ->where("a.doc_key",$doc_key)
            ->where("a.base_type",21) //21=AP Invoice
            ->get();
            foreach($dataAPInvoice as $recAPInvoice) {
                //Update AP Invoice
            ...
            //AP Credit
            $dataAPCredit= BankKeluar2::from("t_bank_keluar2 as a")
            ->leftJoin("t_ap_credit1 as b","a.base_ref","=","b.doc_key")
            ->selectRaw("b.doc_key, b.rp_sisa, a.rp_bayar, a.rp_diskon")
            ->where("a.doc_key",$doc_key)
            ->where("a.base_type",22) //22=AP Credit
            ->get();
            foreach($dataAPCredit as $recAPCredit) {
                //Update AP Credit
    ...
    public function generateJurnal($doc_key = 0, $user_id = '') {
        ...
        $docTrans=61; //Bank Keluar

        //Hapus Jurnal Lama
        AccountDtl::where('base_doc_key',$doc_key)->delete();

        //Jurnal Purcase Invoice
        ...
        //Update Doc Key Jurnal di BankKeluar1
        BankKeluar1::where('doc_key',$recJurnal->doc_key)->update(['doc_key_jurnal'=>$doc_key]);
    }
    ...
    public function store(Request $request) {
        ...
        $dataTrans1= $data['t_bank_keluar1'];
        ...
        try {
            ...
            $bankKeluar1= BankKeluar1::where('doc_key',$where['doc_key'])->first();
            if ($bankKeluar1) {
                BankKeluarController::updateLinkData($doc_key, FALSE);
            } else {
                $bankKeluar1= new BankKeluar1();
            ...
            //Data BankKeluar2
            $existingIds = BankKeluar2::where('doc_key',$doc_key)->pluck('dtl2_key')->toArray();
            $newIds = collect($dataTrans2)->pluck('dtl2_key')->filter()->toArray();
            // Delete items that are not in request
            $toDelete = array_diff($existingIds, $newIds);
            BankKeluar2::whereIn('dtl2_key', $toDelete)->delete();

            //Data BankKeluar2
            //BankKeluar2::where('doc_key',$where['doc_key'])->delete(); //Hapus data existing
            foreach($dataTrans2 as $recTrans2) {
                ...
                $bankKeluar2 = BankKeluar2::where('dtl2_key',$recTrans2['dtl2_key'])->first();
                if (!($bankKeluar2)) {
                    $bankKeluar2 = new BankKeluar2();
