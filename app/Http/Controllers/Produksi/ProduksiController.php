    ...
    public static function updateStok($doc_key = 0, $insert = FALSE) {
        ...
        if ($insert == FALSE) {
            ...
            foreach($dataProduksi1 as $recProduksi1) {
                //FIFO Header
                $dataStokFifo= StokFifo::where("stok_fifo_key",$recProduksi1->stok_fifo_key)->first();
                if ($dataStokFifo) {
                    $dataStokFifo->qty_on_hand = $dataStokFifo->qty_on_hand - $recProduksi1->qty;
                    $dataStokFifo->qty_in = $dataStokFifo->qty_in - $recProduksi1->qty;
                    $dataStokFifo->save();
                    //FIFO Detail
            ...
            foreach($dataProduksi1 as $recProduksi1) {
                //FIFO Header
                $dataStokFifo= StokFifo::where("kd_lokasi",$recProduksi1->kd_lokasi)
                    ->where("kd_bahan",$recProduksi1->kd_bahan)
                    ->where("satuan",$recProduksi1->satuan)
                    ->where("base_type",$docTrans)
                    ->where("base_doc_key",$recProduksi1->doc_key)
                ...
                $dataStokFifo->satuan = $recProduksi1->satuan;
                $dataStokFifo->tgl_doc = $recProduksi1->tgl_doc;
                $dataStokFifo->qty_on_hand = $dataStokFifo->qty_on_hand + ($recProduksi1->qty);
                $dataStokFifo->qty_in = $dataStokFifo->qty_in + ($recProduksi1->qty);
                $dataStokFifo->rp_harga = $recProduksi1->rp_harga;
                ...
                $dataStokFifoDtl->base_doc_key = $recProduksi1->doc_key;
                //$dataStokFifoDtl->base_dtl2_key = $recProduksi1->dtl2_key;
                $dataStokFifoDtl->save();
            }
            //Bahan/Komponen yang dipakai
            $qty= 0;
            $qtyStok=0;
